...
switch (yytype)
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YY_ASSERT (tok == 0 || tok == 10 || tok == 44 || tok == 58 || tok == 40 || tok == 41 || tok == 43);
      }
#else
      ...
      {
        YY_ASSERT (tok == token::MNEMONIC || tok == token::OTHERS || tok == token::SYMBOL || tok == token::IMMSYMBOL || tok == token::IMMNUMBER || tok == token::REGISTER || tok == token::IDENTIFIER || tok == token::NUMBER);
      }
#else
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_IMMSYMBOL (std::string v, location_type l)
      {
        return symbol_type (token::IMMSYMBOL, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_IMMSYMBOL (const std::string& v, const location_type& l)
      {
        return symbol_type (token::IMMSYMBOL, v, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_IMMNUMBER (std::string v, location_type l)
      {
        return symbol_type (token::IMMNUMBER, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_IMMNUMBER (const std::string& v, const location_type& l)
      {
        return symbol_type (token::IMMNUMBER, v, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
