# Author        : OV Shashank
# Date          : 04-11-2016
# Title         : Assemble Function
# Description   : Module altered to work with the 
# serial bootloader. Returns list of bytes to be sent
...
def assemble(input_file):
    ...
    TOKEN_DATA = {
      ...
      'ado': (OP_CODE, VALUE, COMMA, VALUE, COMMA, VALUE),
      ...
      'ndo': (OP_CODE, VALUE, COMMA, VALUE, COMMA, VALUE),
    ...
    INS_DATA = {
      ...
      'ado': ("0000", REGISTER, REGISTER, REGISTER, "011\n"),
      ...
      'ndo': ("0010", REGISTER, REGISTER, REGISTER, "011\n"),
    ...
    while hex_output:
      ...
      line = ""
      if(branch+1>32):
        line += dec_to_hex(64/2,2) + dec_to_hex(address,4) + "".join(hex_output[0:32])
        hex_output = hex_output[32:]
        address += 32;
      else:  
        line += dec_to_hex((branch),2) + dec_to_hex(address,4) + "".join(hex_output[0:branch])
    ...
    print(len(file_hex))
    file_boot = [];
    for i in range(0, len(file_hex), 2):
      print(i," ",hex_to_dec(file_hex[i:i+2]))
      file_boot.append(hex_to_dec(file_hex[i:i+2]))
    return file_boot